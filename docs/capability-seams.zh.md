...
flowchart LR
  ...
  svc_fileJournal["ctx.fileJournal<br/>Workspace file rewind"]
  pkg_api_session_controller["api-session-controller"]
  pkg_api_task_feedback["api-task-feedback"]
  svc_taskFeedback["ctx.taskFeedback<br/>Dispatched task outcomes"]
  pkg_hmr["hmr"]
  ...
  pkg_api_task_feedback --> svc_taskFeedback
  ...
  pkg_session_rewind_files --> svc_fileJournal
  ...
  svc_fileJournal --> pkg_api_session_controller
...
| --- | --- | --- | --- | --- | --- | --- |
| `ctx.fileJournal` | `service` | [`session-rewind-files`](../packages/session/session-rewind-files) | - | [`api-session-controller`](../packages/api/session-controller) | - | 记录已验证的工作区文件写入，恢复明确选择的回合，并保留发生冲突的外部修改。 |
| `ctx.taskFeedback` | `service` | [`api-task-feedback`](../packages/api/task-feedback) | - | - | - | 管理任务回执与有界的结果投递；接收 Agent 负责审阅和验收。 |
