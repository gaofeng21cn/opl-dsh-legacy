...
导出的 `PERSONA_PREFIX_SECTION`（`deployment:persona-prefix`）与 `PERSONA_SUFFIX_SECTION`（`deployment:persona-suffix`）为全局配置和带作用域贡献所共享的段落命名。它们对应的 `PromptSectionOrderName` 项为 `DEPLOYMENT_PERSONA_PREFIX` 与 `DEPLOYMENT_PERSONA_SUFFIX`；[包 README](../../packages/core/system-prompt/README.zh.md#configure-the-prompt)规定其位置与模板配置。`OUTPUT_LANGUAGE_SECTION`（`harness:output-language`，顺序项 `OUTPUT_LANGUAGE`）承载用户选定的输出语言；[包 README](../../packages/core/system-prompt/README.zh.md#select-the-output-language)规定其设置段与渲染出的指令。
