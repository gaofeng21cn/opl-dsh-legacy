---
description: "完整的 Session V4 持久化类型与物理编码参考。"
...
这份不可变归档记录 V5 rewind 来源扩展之前的 V4 Session 写入器，保留 V4→V5 相邻迁移使用的完整声明清单。
...
清单来自 rewind 消息来源扩展之前捕获的 V4 写入器检查点。当前写入器和发布状态由配套的 Session 格式状态页记录。
...
[完整机器目录](v4.schema.json)包含 61 个根类型和 467 种可达类型。摘要包含所有引用字段；源码名称和路径描述所选的历史源码树。
...
[`event:agent/inbox/spliced.data.inserted.item.source.variant-12.references.item`](#persistence-type-eventagentinboxspliceddatainserteditemsourcevariant-12referencesitem) 的数组。
...
[`event:agent/inbox/spliced.data.inserted.item.source.variant-4.entries.item`](#persistence-type-eventagentinboxspliceddatainserteditemsourcevariant-4entriesitem) 的数组。
...
| `kind` | 必需 | `"plugin"` |
| `plugin` | 必需 | `string` |
| `sections` | 必需 | [`event:agent/inbox/spliced.data.inserted.item.source.variant-6.sections`](#persistence-type-eventagentinboxspliceddatainserteditemsourcevariant-6sections) |
...
来源：`packages/compaction/compaction/src/types.ts` · `packages/compaction/compaction/src/types.ts`
...
来源：`packages/core/session/src/types.ts` · `packages/core/session/src/types.ts`
...
| `name` | 可选 | `string` |
| `originalDimensions` | 可选 | [`event:agent/inbox/spliced.data.inserted.item.content.item.variant-1.attachment.originalDimensions`](#persistence-type-eventagentinboxspliceddatainserteditemcontentitemvariant-1attachmentoriginaldimensions) |
| `width` | 必需 | `number` |
...
| 属性 | 存在性 | 类型 |
|---|---|---|
| `action` | 必需 | [`event:agent/inbox/spliced.data.inserted.item.source.variant-0.changes.item.action`](#persistence-type-eventagentinboxspliceddatainserteditemsourcevariant-0changesitemaction) |
| `digest` | 可选 | `string` |
| `path` | 必需 | `string` |
| `scope` | 必需 | `string` |
...
| `changes` | 必需 | [`event:agent/inbox/spliced.data.inserted.item.source.variant-0.changes`](#persistence-type-eventagentinboxspliceddatainserteditemsourcevariant-0changes) |
| `form` | 必需 | `"instructions"` |
...
| `references` | 必需 | [`event:agent/inbox/spliced.data.inserted.item.source.variant-12.references`](#persistence-type-eventagentinboxspliceddatainserteditemsourcevariant-12references) |
| `version` | 必需 | `1` |
...
来源：`packages/core/session/src/types.ts` · `packages/interaction/commands/src/types.ts` · `packages/interaction/commands/src/types.ts` · `packages/llm/llm/src/message.ts` · `packages/session/session-title/src/types.ts`
...
| `id` | 必需 | `string` |
| `role` | 必需 | `"system"` |
| `source` | 必需 | [`event:session/title-llm-request.data.messages.item.variant-1.source`](#persistence-type-eventsessiontitle-llm-requestdatamessagesitemvariant-1source) |
...
| 属性 | 存在性 | 类型 |
|---|---|---|
| `entries` | 必需 | [`event:agent/inbox/spliced.data.inserted.item.source.variant-4.entries`](#persistence-type-eventagentinboxspliceddatainserteditemsourcevariant-4entries) |
| `form` | 必需 | `"catalog"` |
...
来源：`packages/api/session-controller/src/types.ts` · `packages/attachment/attachment/src/brand.ts` · `packages/compaction/compaction/src/brand.ts` · `packages/core/session/src/types.ts` · `packages/experimental/agent-team/src/types.ts` · `packages/experimental/agent-team/src/types.ts` · `packages/experimental/agent-team/src/types.ts` · `packages/feedback/message-feedback/src/types.ts` · `packages/goal/goal/src/types.ts` · `packages/interaction/commands/src/brand.ts` · `packages/interaction/user-approval/src/types.ts` · `packages/llm/llm-retry/src/brand.ts` · `packages/llm/llm/src/brand.ts` · `packages/llm/llm/src/brand.ts` · `packages/llm/llm/src/brand.ts` · `packages/llm/llm/src/brand.ts` · `packages/schedule/schedule/src/types.ts` · `packages/session/session-title/src/index.ts` · `packages/session/session-title/src/types.ts` · `packages/webhook/webhook/src/brand.ts` · `packages/webhook/webhook/src/brand.ts` · `packages/webhook/webhook/src/brand.ts` · `packages/workflow/workflow/src/types.ts`
...
该 schema 作为历史参考保留，不从 V5 写入器重新生成。V4 物理行由已发布的 V4 编解码器读取；当前读取器打开时再通过 V4→V5 相邻恒等迁移。源文件不会被覆盖。
