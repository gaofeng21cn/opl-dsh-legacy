...
The inventory is the complete schema committed in the upstream DeepSeek Harness release tag `dsh-v0.1.7-rc.2`. The tag belongs to the upstream repository; the OPL fork uses separate `opl-` release tags. The current writer and release status are recorded in the paired Session format status page.
