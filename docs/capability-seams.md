...
| --- | --- | --- | --- | --- | --- | --- |
| `ctx.fileJournal` | `service` | [`session-rewind-files`](../packages/session/session-rewind-files) | - | [`api-session-controller`](../packages/api/session-controller) | - | Records verified workspace writes and restores an explicitly selected turn without overwriting conflicting external changes. |
| `ctx.taskFeedback` | `service` | [`api-task-feedback`](../packages/api/task-feedback) | - | - | - | Owns task receipts and bounded outcome delivery; receiving agents own review and acceptance. |
