...
export function verifyOplAppBundle(appPath, options = {}) {
  ...
  if (!existsSync(join(runtimeDir, 'node', 'node'))) {
    throw new Error(`OPL package verification: the bundled runtime has no node in ${runtimeDir}`)
  }
...
export function verifyOplWindowsApplication(appOutDir, options = {}) {
  ...
  verifyBundledRuntime(runtimeDir)
  if (!existsSync(join(runtimeDir, 'node', 'node.exe'))) {
    throw new Error(`OPL package verification: the bundled runtime has no node.exe in ${runtimeDir}`)
  }
