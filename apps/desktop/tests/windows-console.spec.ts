...
const { DesktopProjectManager } = await import('../src/project-manager.ts')
...
it('hides each bundled pnpm package transaction', async () => {
  const root = scratch()
  ...
  const manager = new DesktopProjectManager({
    ...
    pnpm: {
      root: join(root, 'desktop', 'pnpm'),
      store: join(root, 'desktop', 'pnpm', 'store'),
      cache: join(root, 'desktop', 'pnpm', 'cache'),
      state: join(root, 'desktop', 'pnpm', 'state'),
      config: join(root, 'desktop', 'pnpm', 'config'),
      home: join(root, 'desktop', 'pnpm', 'home'),
    },
  }, { node: process.execPath, pnpm: join(root, 'pnpm.mjs'), dsh: root })
  const runPnpm = (manager as unknown as {
    runPnpm: (projectDir: string, args: readonly string[]) => Promise<void>
  }).runPnpm.bind(manager)
  await expect(runPnpm(profile, ['install', '--frozen-lockfile'])).rejects.toThrow('spawn recorded for the console contract')
  expectBackgroundConsole(calls.at(-1))
  expect(calls.at(-1)?.command).toBe(process.execPath)
})
