...
describe.skipIf(process.platform !== 'win32')('Restricted Git Bash (real Windows backend)', () => {
  it('refuses a confined launch with the unproven dimension and never runs the command', async () => {
    ...
    for (const mode of ['workspace-write', 'read-only'] as const) {
      ...
      const foreground = await ctx.shell.run(spec).catch((error: unknown) => error)
      ...
      await expect(ctx.shell.start(spec)).rejects.toMatchObject({ code: SANDBOX_UNAVAILABLE })
    }
  ...
  it('names the real MSYS failure the probe observed', async () => {
    ...
    mkdirSync(workspace)
    const ctx = await boot('workspace-write', workspace)
    const result = await refusalMessage(ctx.shell.run(ctx.shell.resolve({ workdir: workspace, command: 'true' })))
  ...
  it('refuses a launch directory outside the granted roots before probing', async () => {
    ...
    const refusal = await refusalMessage(ctx.shell.run(ctx.shell.resolve({ workdir: process.cwd(), command: 'true' })))
  ...
  it('refuses foreign drive mounts, traversal, and device spellings before probing', async () => {
    ...
    for (const workdir of workdirs) {
      const refusal = await refusalMessage(ctx.shell.run(ctx.shell.resolve({ workdir, command: 'true' })))
    ...
    const foreign = await refusalMessage(ctx.shell.run(ctx.shell.resolve({ workdir: '/mnt/c/Users', command: 'true' })))
  ...
  it('treats a junction that leaves the workspace as outside the boundary', async () => {
    ...
    const ctx = await boot('workspace-write', workspace)
    const throughLink = await refusalMessage(ctx.shell.run(ctx.shell.resolve({ workdir: link, command: 'true' })))
    ...
    const inside = await refusalMessage(ctx.shell.run(ctx.shell.resolve({ workdir: workspace, command: 'true' })))
  ...
  it('keeps the confined preset unavailable while an approved full-access call still runs', async () => {
    ...
    await expect(ctx.shell.run(ctx.shell.resolve({ workdir: workspace, command: 'printf a > inside.txt' })))
    ...
    const approved = await ctx.shell.run(ctx.shell.resolve({
      workdir: workspace,
  ...
  it('sheds a background Bash process and its descendants when killed', async () => {
    ...
    const pidFile = join(workspace, 'descendant.pid')
    const ctx = await boot('read-only', workspace)
    const proc = await ctx.shell.start(ctx.shell.resolve({
      workdir: workspace,
      command: `sleep 300 & child=$!; { cat "/proc/$child/winpid" 2>/dev/null || printf '%s' "$child"; } > '${pidFile}'; wait`,
      sandboxPolicy: { mode: 'danger-full-access', workspaceRoot: workspace },
    }))
  ...
  it('leaves no writer behind when a read-only refusal follows an approved run', async () => {
    ...
    const refused = await refusalMessage(ctx.shell.run(ctx.shell.resolve({ workdir: workspace, command: 'printf x > refused.txt' })))
