...
async function loadNative(home: string): Promise<Context> {
  ...
  writeFileSync(join(home, 'providers.mjs'), [
    ...
    ' await ctx.plugin(providers.settings, { dshHome: config.home, watch: false })',
    '}',
  ].join('\n'))
  ...
  const ctx = await boot('git-bash-test', join(home, 'cordis.yml'), undefined, (ctx) => {
    ctx.provide('nativeShellProviders', {
      subprocess: LocalSubprocessRuntime, settings: FileSettingsProvider, bash: LocalBashExecutor, pwsh: PwshLocalExecutor,
    })
  })
  contexts.push(ctx)
  return ctx
...
describe.skipIf(process.platform !== 'win32')('Native Git Bash', () => {
  ...
  it('boots stored settings, runs native tools and retains the startup shell until restart', async () => {
    ...
    await ctx.settings.update('shell', { agentShell: 'powershell', gitBashPath: 'C:/not-installed/bash.exe' })
    expect(ctx.get('dshAgentShell')?.()).toBe('git-bash')
    expect(ctx.get('dshGitBashPath')?.()).toBe(git)
    expect(bash.bashPath).toBe(git)
    expect((await bash.run(bash.resolve({ command: "printf '%s' bash-still-active" }))).stdout.text).toBe('bash-still-active')
  ...
  it('keeps PowerShell by default and rejects an unavailable Git Bash', async () => {
    ...
    await expect(ctx.settings.update('shell', { agentShell: 'git-bash', gitBashPath: 'C:/missing/bash.exe' }))
  ...
  it('settles deadlines and cancellation through the managed process provider', async () => {
    ...
    writeFileSync(join(home, 'settings.yaml'), 'shell:\n  agentShell: git-bash\n')
    const ctx = await loadNative(home)
    const timeout = await ctx.shell.run(ctx.shell.resolve({ command: 'sleep 30', timeoutMs: 300 }))
    expect(timeout.timedOut).toBe(true)
    const abort = new AbortController()
    const running = ctx.shell.run(ctx.shell.resolve({ command: 'sleep 30', signal: abort.signal }))
  ...
  it('refuses restricted Git Bash before starting any command; explicit full access still works', async () => {
    ...
    for (const mode of ['workspace-write', 'read-only'] as const) {
      ...
      await expect(ctx.shell.run(spec)).rejects.toThrow('Use PowerShell for read-only/workspace-write')
      await expect(ctx.shell.start(spec)).rejects.toThrow('permissions were not changed')
      expect(existsSync(join(root, 'outside.txt'))).toBe(false)
    }
    const allowed = await ctx.shell.run(ctx.shell.resolve({ workdir: workspace, command: 'printf ok > allowed.txt',
