...
The coordinator needs an OPL DSH desktop with task feedback and permission services, and its control client (installed as `scripts/control.mjs`). The control script reads the desktop profile and uses its loopback endpoint and token internally. It does not start the desktop application. DSH continues to own the model, tools, permissions, and worker session; the Skill supplies the Codex coordination workflow.

Open **OPL DSH → Settings → Codex collaboration** and click **Install skill**. The desktop installs its bundled files into `$CODEX_HOME/skills/opl-dsh-workflow`, or `~/.codex/skills/opl-dsh-workflow` when `CODEX_HOME` is unset. The displayed path is authoritative. It also writes a private `coordinator.json` beside `SKILL.md`. Installing the DMG alone does not install the Skill. Open a new Codex task after installation; if necessary, restart Codex. The settings page can update the files and toggle automatic DSH launch. It preserves custom config values and refuses to overwrite modified or unmanaged Skill files. No API keys or control tokens are copied.

The installed helper defaults to this configuration regardless of its current working directory. It requires Node.js to run; the installed config uses the desktop executable in Electron Node mode for the bundled control client only. GUI startup explicitly removes that environment flag. The current Codex task comes from `--thread`, then `targetThreadId` in the config, then `CODEX_THREAD_ID`. A missing real task ID is an error. The installer enables `ledgerPerThread` so unrelated Codex tasks use separate ledgers. Moving the application may require another settings-page update to refresh its executable path.

For manual setup, create a private JSON config outside version control and pass `--config /absolute/path/to/config.json`. Required fields are `controlCli` and `ledgerDir`, plus a real thread ID from one of the sources above. `node`, `dshHome`, `timeoutMs`, `pathMode`, `electronNode`, and `ledgerPerThread` are optional. Both new switches are booleans; omission preserves the previous behavior. To let the Skill start DSH, add `startCommand`; `startArgs`, `startCwd`, and `startupTimeoutMs` are optional companions. Paths in the config must be absolute native paths; `node` and `startCommand` may be executables on `PATH`. The control timeout is a positive integer in milliseconds, at most 86400000; omission uses 150000. The startup timeout is at most 120000ms and defaults to 30000ms. Unknown fields are rejected.
