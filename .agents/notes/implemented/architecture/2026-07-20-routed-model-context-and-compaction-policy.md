...
For proactive pressure, compaction-basic reads the latest durable request route, resolves its adapter capacity and exact-target policy, clamps the configured effective input budget to that capacity, and scales ratios into a `ResolvedCompactSpec`. It performs this resolution on every check, so a provider or model switch in one session changes capacity and policy immediately. An absolute retained budget that is not below the scaled threshold fails when the target capacity first makes that comparison possible — or at load when a configured `inputBudget` already decides the trigger. The [routed input budget Agent Note](../feature/2026-09-23-routed-input-budget-and-step-admission.md) owns the budget's own contract.
