/** One-time contributor command to publish V5 successors through JSONL persistence. */
...
const usage = `Usage: pnpm run migrate:sessions-to-v5 [--sessions-dir PATH] [--jobs N]

Publish V5 successors beside unchanged historical Session generations.
Defaults to ~/.dsh/sessions. Already-V5 Sessions are opened read-only.
...
function emptyCounts(): OutcomeCounts {
  return { converted: 0, alreadyV5: 0, failed: 0, skipped: 0 }
}
...
async function migrate(root: string, jobs: number): Promise<number> {
  const reportDirectory = mkdtempSync(join(tmpdir(), 'dsh-migrate-v5-'))
  ...
  try {
    write(`Session migration to V5 started ${startedAt}`)
    write(`Root: ${root}`)
    ...
    assert.equal(SESSION_FORMAT_VERSION, 5, 'this one-time command requires a V5 Session writer')
    assert.equal(sessionFormatCatalog.currentVersion, 5, 'this one-time command requires a V5 format catalog')
    ...
    try {
      ...
      await runMigrationJobs(candidates.length, jobs, async (index, retry) => {
        ...
        try {
          ...
          const handle = await ctx.sessionPersistence.open(id, source.version === 5 ? 'read' : 'write')
          await handle.close()
          recordOutcome(source.version, source.version === 5 ? 'alreadyV5' : 'converted')
          finish(`${source.version === 5 ? 'already V5 (opened successfully)' : `V${source.version} -> V5: ${generationLogFilename(5, source.compression)}`} (${((performance.now() - started) / 1000).toFixed(2)}s)`)
        } catch (error: unknown) {
    ...
    try {
      write(`Summary: converted=${totals.converted}, already-V5=${totals.alreadyV5}, failed=${totals.failed}, skipped=${totals.skipped}`)
