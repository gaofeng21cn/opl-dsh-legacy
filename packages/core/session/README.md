...
Surface events (`system/message`, `user/message`, `assistant/message`, `tool/result`) require `surfaceOp` in both typed events and append input. A replacement uses exactly `{ op: 'replace', startSeq, endSeq }`, with inclusive `SessionSeq` endpoints in current surface order. An Assistant message embeds its exact compact provider stream and forbids `sourceEventSeqs`. Known log-only events forbid both metadata fields and never produce a message. `isAppendSurfaceEvent` and `isReplacementSurfaceEvent` split the two markers, and `isRewindSurfaceEvent` recognizes the conversation-rewind convention on top of a replacement: an empty `system/message` whose `source.plugin` is `REWIND_SURFACE_PLUGIN`. That node keeps the removed prompt's surface position while deriving no message, so a reader unaware of rewinds still folds the log into the same model history.
