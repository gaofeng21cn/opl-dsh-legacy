...
`isAppendSurfaceEvent` and `isReplacementSurfaceEvent` split the two markers, and `isRewindSurfaceEvent` recognizes the conversation-rewind convention on top of a replacement: an empty `developer/message` whose `source.kind` is `REWIND_SURFACE_PLUGIN`. That node keeps the removed prompt's surface position while deriving no message, so a reader unaware of rewinds still folds the log into the same model history.
