...
/**
 * Machine-readable transport failure classes a request can end in.
 *
 * A `TRANSPORT` failure is one code covering several distinct causes, and the
 * operator-facing difference matters: a refused connection is a network or
 * endpoint problem, an abandoned read is a timeout, and neither is a provider
 * rejection. The adapter that owns the connection reports which one it saw.
 */
export type LlmTransportStage =
  /**
   * `fetch` itself rejected: DNS, TCP, TLS, or waiting for the response
   * headers. Which of those it was is carried by {@link LlmFailure.causeCode}.
   */
  | 'request'
  /** The response arrived and its body failed part-way through the stream. */
  | 'response-body'
