...
const totalsSchema = z.object({ calls: z.number().nonnegative(), succeeded: z.number().nonnegative(), failed: z.number().nonnegative(), durationMs: z.number().nonnegative(), inputTokens: z.number().nonnegative(), outputTokens: z.number().nonnegative(), cachedTokens: z.number().nonnegative(), unknownUsage: z.number().nonnegative() })
const stateSchema = z.object({ preferences: preferencesSchema, totals: totalsSchema, buckets: z.array(totalsSchema.extend({ mode: z.enum(['cloud', 'local']), model: z.string(), sessionId: z.string().nullable() })).max(2000) })
const empty = (): OplSearchTotals => ({ calls: 0, succeeded: 0, failed: 0, durationMs: 0, inputTokens: 0, outputTokens: 0, cachedTokens: 0, unknownUsage: 0 })
...
export class OplSearchService extends TypertRemoteService {
  ...
  async test(preferences: OplSearchPreferences, query: string, signal: AbortSignal): Promise<OplSearchTestResult> {
    ...
    return { sources: result.sources.map(source => ({ url: source.url, ...(source.title ? { title: source.title } : {}) })), durationMs: Date.now() - start }
  }
  ...
  private execute(request: WebSearchRequest, signal: AbortSignal | undefined, preferences: OplSearchPreferences, sessionId: string | null): Promise<WebSearchResult> {
  ...
  private async executeTracked(request: WebSearchRequest, signal: AbortSignal | undefined, preferences: OplSearchPreferences, sessionId: string | null): Promise<WebSearchResult> {
    ...
    try {
      const result = preferences.mode === 'local'
        ...
        : await new OplGatewaySearchProvider(() => ({ ...this.options.cloud(), model: preferences.model, recordUsage: value => { usage = value } })).search(request, budget)
      if (result.sources.length === 0) throw new Error('Search returned no sources.')
      succeeded = true
      return result
    } finally {
      this.pending.delete(controller)
      const delta: OplSearchTotals = { calls: 1, succeeded: succeeded ? 1 : 0, failed: succeeded ? 0 : 1, durationMs: Date.now() - started, inputTokens: usage?.inputTokens ?? 0, outputTokens: usage?.outputTokens ?? 0, cachedTokens: usage?.cachedTokens ?? 0, unknownUsage: preferences.mode === 'cloud' && (usage?.inputTokens === undefined || usage.outputTokens === undefined) ? 1 : 0 }
