...
|---|---|---|
| `apiKeyEnv` | `OPL_GATEWAY_API_KEY` | Credential reference resolved per request; the account page writes this reference after sign-in. |
...
The adapter subclasses the DeepSeek adapter and keeps Chat Completions request assembly, streaming, and token accounting. The OPL search service separately selects local Bing retrieval or account-authenticated Responses search. The upstream DeepSeek search provider remains independent. See the [desktop search settings](../../../apps/desktop/README.md#desktop-control).

| File | Responsibility |
|---|---|
| `src/config.ts` | Config schema, the `OPL_GATEWAY_API_KEY` default, the advertised model, and translation into adapter options. |
...
- [settings-file](../../settings/settings-file/README.md) — the document a deployment or the Models page writes the section into.
