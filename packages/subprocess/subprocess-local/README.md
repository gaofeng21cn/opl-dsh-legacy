...
Mount the provider beside its consumers and start processes exactly as the subprocess service specifies; this package decides only how those processes run on the host. On Windows, non-terminal children and `taskkill` helpers start with their windows hidden so background operations do not take focus: the Job runner hides its own console whenever its image has one, and every ordinary target owns a hidden console, which also covers a packaged runner whose GUI image has no console to pass on. This also hides GUI windows that honor the process startup visibility setting.
