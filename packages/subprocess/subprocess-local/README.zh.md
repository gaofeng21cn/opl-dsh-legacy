...
把提供方与它的消费方挂载在同一组合中，并完全按子进程服务的规定启动进程；本包只决定这些进程在宿主机上如何运行。在 Windows 上，非终端子进程与 `taskkill` 辅助进程会隐藏窗口，因此后台操作不会抢占焦点：Job runner 只要自身映像拥有控制台就会隐藏它，而每个普通目标都拥有隐藏控制台——这也覆盖了打包版 runner 的 GUI 映像无可传递控制台的情况。遵循进程启动可见性设置的 GUI 窗口也会被隐藏。
