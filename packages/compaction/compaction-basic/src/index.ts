...
export class BasicCompactionEngine extends CompactionEngine {
  ...
  private async stepRefusal(
    ...
    if (error instanceof PressureCompactionError) {
      if (!bindsBelowCapacity(error.inputBudget, error.contextWindow)) return undefined
    ...
    // Neither an adapter capacity nor a configured budget: the provider's own
    // context-overflow recovery stays the only authority for this route.
    if (capacity === undefined) {
      return configured === undefined
        ? undefined
        : this.stepBudgetRefusal(agent, error, configured, `${target.provider}/${target.model}`)
    }
    const inputBudget = Math.min(capacity, configured ?? capacity)
    if (!bindsBelowCapacity(inputBudget, capacity)) return undefined
