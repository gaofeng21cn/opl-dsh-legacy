...
Native Git Bash 选项也会为 minimal 预设选择本后端。它接收启动时的 Git Bash 路径，保留交互式 Bash 状态，不读取后续编辑的 shell 选项。Windows 受限模式会在分配 Bash 终端前拒绝：MSYS 会初始化安全描述符只列出用户 SID 的每用户运行时对象（共享内存映射与信号管道），受限令牌的写入检查永远匹配不到它们，而会话 shell 也无法由一次性 broker 的能力探针授权。PowerShell 保留受限模式支持，静态拒绝使该决定不依赖任何需要探针建立的主机事实。
