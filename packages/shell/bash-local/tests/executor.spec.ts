...
describe('LocalBashExecutor.run', () => {
  ...
  it('settles a cancellation that lands inside the provider\'s own pre-spawn check', async () => {
    ...
    const result = await run(bash, bash.resolve({ command: 'sleep 60', signal: controller.signal }))
    expect(result.aborted).toBe(true)
    expect(result.timedOut).toBe(false)
  ...
  it('settles a deadline that expires inside the provider\'s own pre-spawn check', async () => {
    ...
    try {
      ...
      const result = await run(bash, bash.resolve({ command: 'sleep 60', timeoutMs: 10 }))
      expect(result.timedOut).toBe(true)
      expect(result.aborted).toBe(false)
      expect(result.exitCode).toBeNull()
    } finally {
