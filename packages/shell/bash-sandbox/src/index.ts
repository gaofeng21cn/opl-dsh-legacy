...
export class SandboxBashExecutor extends LocalBashExecutor {
  ...
  override async run(spec: ShellExecSpec): Promise<ShellRunResult> {
    ...
    try {
      ({ result, spawnRequested } = await this.runArgv(guarded, async (signal) => {
  ...
  override async start(spec: ShellExecSpec): Promise<ShellProcess> {
    ...
    const guarded = this.guardedSpec(spec, { ...policy, mode })
    const confined = await this.confine(guarded, { ...policy, mode }, spec.signal)
    spec.signal?.throwIfAborted()
    let proc: ShellProcess
    try {
      proc = this.startArgv(guarded, confined.argv)
    } catch (error) {
      ...
      if (isRunnerSpawnFailure(error, confined.argv[0], guarded.workdir)) {
        throw new SandboxUnavailableError(mode, String(error))
      }
      throw error
    }
  ...
  private async runProbeCommand(
    ...
    try {
      const { result, spawnRequested } = await this.runArgv(probeSpec, [...wrappedArgv, GIT_BASH_PROBE_ARGV0, ...args])
      if (!spawnRequested) return { exitCode: null, stderr: 'the capability probe timed out before it could start' }
