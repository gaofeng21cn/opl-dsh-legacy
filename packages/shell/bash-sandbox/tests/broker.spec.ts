...
const spillDir = mkdtempSync(join(tmpdir(), 'dsh-git-bash-broker-spec-'))
/** Real directories stand in for the execution-world workspace on any host. */
const scratch: string[] = []

function temporary(): string {
  const dir = mkdtempSync(join(tmpdir(), 'dsh-broker-ws-'))
  scratch.push(dir)
  return dir
}
...
describe('the Git Bash broker launch path on win32', () => {
  it('runs the guarded spec once the probe proves every dimension', async () => {
    ...
    const { ctx, bash, recorder } = await setup({
      ...
      internals: { platform: 'win32', probeGitBash: probe, tempRoot: tmpdir() },
    })
    recordSpawns(ctx, recorder)
    try {
      const result = await bash.run(bash.resolve({ command: 'printf ok', workdir: workspace }))
  ...
  it('refuses an unproven host without wrapping or starting anything', async () => {
    ...
    const { ctx, bash, recorder } = await setup({
      mode: 'workspace-write',
      workspaceRoot: workspace,
      internals: { platform: 'win32', probeGitBash: async () => PARTIAL, tempRoot: tmpdir() },
    })
    recordSpawns(ctx, recorder)
    try {
      for (const operation of ['run', 'start'] as const) {
        await expect(bash[operation](bash.resolve({ command: 'printf escaped > ../outside.txt', workdir: workspace })))
          .rejects.toMatchObject({ name: 'SandboxUnavailableError', code: SANDBOX_UNAVAILABLE })
      }
      await expect(bash.run(bash.resolve({ command: 'true', workdir: workspace })))
  ...
  it('refuses a launch directory outside the granted roots before probing', async () => {
    ...
    try {
      ...
      await expect(bash.run(bash.resolve({ command: 'true', workdir: outside })))
        .rejects.toThrow(/outside the granted roots/u)
      await expect(bash.start(bash.resolve({ command: 'true', workdir: outside })))
  ...
  it('publishes a background process through the same guarded launch', async () => {
    ...
    const { ctx, bash, recorder } = await setup({
      mode: 'workspace-write',
      workspaceRoot: workspace,
      internals: { platform: 'win32', probeGitBash: async () => PROVEN, tempRoot: tmpdir() },
    })
    recordSpawns(ctx, recorder)
    try {
      const proc: ShellProcess = await bash.start(bash.resolve({ command: 'sleep 1', workdir: workspace }))
      await proc.done
  ...
  it('proves one host once per executable, mode, and workspace', async () => {
    ...
    recordSpawns(ctx, recorder)
    try {
      await bash.run(bash.resolve({ command: 'true', workdir: workspace }))
      await bash.run(bash.resolve({ command: 'true', workdir: workspace }))
...
describe('the Git Bash capability probe', () => {
  ...
  function hostTarget(msysPath: string): string {
    return process.platform === 'win32' ? (msysPathToWindows(msysPath, { tempRoot: tmpdir() }) ?? msysPath) : msysPath
  }
  ...
  async function probe(script: ProbeScript): Promise<{
    ...
    try {
      const result = await bash.run(bash.resolve({ command: 'true', workdir: workspace }))
  ...
  it('keeps the dimension unproven when no base outside the granted roots exists', async () => {
    // The host temp root IS a granted root under workspace-write, so a probe
    // restricted to it cannot observe a write the mode must deny.
    const run = await probe({ startup: { exitCode: 0 }, write: { exitCode: 1 }, scratchRoots: [tmpdir()] })
