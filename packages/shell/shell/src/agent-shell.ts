 ...
 *
 * The choice lives on the shared `shell` settings section, so both shell
 * executors declare the same two fields: a host mounts exactly one of them, and
 * the section must resolve under either selection and on every platform — the
 * same portability reason {@link SHELL_SETTINGS_NAMESPACE} documents. A POSIX
...
/** The `shell` settings fields both shell executors declare. */
 ...
 * @param settings - the resolved `shell` settings section.
 * @param env - the environment to probe; defaults to the process environment.
 * @param platform - the platform to resolve for; defaults to the process platform.
