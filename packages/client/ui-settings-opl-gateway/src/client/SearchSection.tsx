...
export function SearchSection({ t, searchStatus, searchConfigure, searchModels, searchTest }: Props) {
  ...
  useEffect(() => {
    let disposed = false
    void searchStatus().then(value => { if (!disposed) { setStatus(value); setPreferences(value.preferences) } }).catch(e => { if (!disposed) setError(String(e)) })
    ...
    <div className={css.card}>
      ...
      {preferences.mode === 'cloud' && <>
        <label className={css.field}><span>{t('search.model')}</span>
          <select className={css.input} disabled={busy || !status} value={preferences.model} onChange={e => change({ ...preferences, model: e.target.value })}>
