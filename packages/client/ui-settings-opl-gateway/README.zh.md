...
在挂载 `opl-gateway` 模型路由的地方一并挂载它，使该路由的使用者能在应用内登录。若本机的 OPL 安装已记录绑定，缺少本页仍可继续工作——适配器会复用那把密钥——但无人能从 Harness 内登录、刷新或释放密钥。提供直接供应商密钥的部署两者都不挂载。
...
无，因为本包是面向网关账号 Remote 的浏览器侧设置页，不注册任何面向模型的内容。
