...
export const inject = ['sessions', 'remote', 'uiSession']
...
export function apply(ctx: ClientContext): void {
  ...
  const interactions = ctx.uiSession.pendingInteractions
  const publish = (): void => {
    for (const [sessionId, interaction] of interactions.getSnapshot()) {
  ...
  ctx.effect(() => bridge.onActivate((candidate) => {
    ...
    ctx.sessions.open(candidate as SessionId)
