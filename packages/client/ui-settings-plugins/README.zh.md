...
通过共享设置界面提供插件设置与可选的桌面控制项。每个控件读取并更新对应服务，不另建配置存储。
...
本 fork 还会在 Host 提供 `system-prompt` 时向插件页添加「输出语言」；三个选项通过共享配置表单更新 `outputLanguage`，从下一次请求起生效。桌面应用中的「桌面」设置通过 Electron preload 接口保存通知、关闭窗口行为及 Windows 本机或 WSL2 执行环境；切换环境需要重启。
