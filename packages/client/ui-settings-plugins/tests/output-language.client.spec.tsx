...
function renderCard(t: (key: keyof typeof en) => string, state: Partial<OutputLanguageCardState> = {}) {
  ...
  } as unknown as OutputLanguageCardProps
  render(<OutputLanguageCard {...props} />)
  fireEvent.click(screen.getByText(t('outputLanguageTitle')))
...
function acceptWrites(host: StubSettingsScope<OutputLanguageSettings>): void {
...
): { host: StubSettingsScope<OutputLanguageSettings>; face: ReturnType<OutputLanguageCardController['inject']> } {
  const host = stubSettingsScope<OutputLanguageSettings>()
  acceptWrites(host)
  host.publish({
    status: 'ready',
    writable: true,
...
describe('OutputLanguageCard', () => {
  ...
  it('stages the chosen language through the select', () => {
    ...
    expect(actions.edit).toHaveBeenCalledWith('language', 'zh')
  })
  ...
  it('stages a reset once the stored language is overridden', () => {
    ...
    expect(actions.resetField).toHaveBeenCalledWith('language')
  })
...
describe('OutputLanguageCardController', () => {
  it('writes the staged choice into the namespace only on save', async () => {
    ...
    const { host, face } = mount({ value: { language: 'default' } })
    ...
    expect(host.scope.getSnapshot().user).toEqual({ language: 'zh' })
  ...
  it('reads the stored language back on a fresh mount over the same document', async () => {
    const first = mount({ value: { language: 'default' } })
    first.face.edit('language', 'zh')
    first.face.save()
    ...
    const second = mount({ value: { language: 'zh' }, user: { language: 'zh' } })
  ...
  it('keeps Default selectable and resettable over a stored language', async () => {
    const { host, face } = mount({ value: { language: 'zh' }, user: { language: 'zh' } })
    const state = () => face.hooks.outputLanguageCard.getSnapshot()

    face.edit('language', 'default')
    expect(state()).toMatchObject({ dirty: true, language: { text: 'default', overridden: true } })
    face.save()
    await awaitSettled(state)
    expect(host.scope.getSnapshot().user).toEqual({ language: 'default' })
  ...
  it('reports a language the Host did not store as a failed save', async () => {
    ...
    face.edit('language', 'en')
    face.save()

    await vi.waitFor(() => {
      expect(host.set).toHaveBeenCalledWith('language', 'en')
