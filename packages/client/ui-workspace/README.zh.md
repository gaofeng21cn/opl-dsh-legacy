...
侧边栏用**项目内会话**与**项目外会话**两个分区标题分隔会话：项目行及其会话位于前一分区，项目外的会话直接平铺在后一分区标题之下，不再套一层文件夹行。“新会话”会创建具有独立工作目录的正常 Agent 会话，无需选择项目；项目行右侧的“+”在该项目目录创建或复用空白会话。右键点击会话行（或其尾部的 ···）可打开重命名、**移动到项目**（全部已登记项目、移到项目外，或通过目录选择器新建项目）、分叉、置顶与归档；悬停该行则直接显示置顶与归档快捷图标。移动只改变项目归属：文件、工作目录、历史记录与 Agent 配置保持不变。
...
**最近更新**在分组和单列表视图中都按普通 Session 的最近一次用户提示词或 steering（中途引导）时间降序排列。**手动排序**会冻结当前显示顺序，并在活动变化时保持已有位置；新发现的普通 Session 追加到末尾，多条同时到达时按最近更新时间降序排列。返回最近更新会丢弃所有手动位置；再次进入手动排序时，则冻结当时的最近更新顺序。浏览器默认采用最近更新，并在重新加载后记住所选模式。拖拽普通 Session 会在浏览器本地应用移动并选中手动排序。**置顶**是浏览器本地标记，会让会话在自己所属分组内领先，但不会离开该分组：置顶在重新加载后保留，其行始终显示置顶开关，而保存的手动顺序不包含任何置顶 id，因此取消置顶会把该行放回拖拽所给的位置。当前选中的空白**新会话**始终固定在首位且无法拖拽；首条提示词落地后，它成为可拖拽的普通行，在手动排序中保留首位，在最近更新中按当前时间戳排列。折叠分组的拖拽边界按渲染行确定，并把来源行放在中间隐藏行之前，因此拖拽不会隐藏来源行。真实 Workspace、Ungrouped 与单列表的 Session 显示顺序都保留在浏览器本地；Workspace 分组的拖拽顺序仍由 Host 持久化。
...
Session 行内的 Rename 操作打开一个以该行显示标题预填的对话框；确认未修改的标题是有意允许的——这正是把当前自动标题钉住、不再被重新生成覆盖的手势。**移动到项目**对已登记项目或项目外直接改变归属，对必须先登记的项目则打开目录选择器；只有当所选目录无法被采用时，该选择器的对话框才会返回，报告失败并仍然提供全部目标。Archive 可从行的快捷图标或菜单直接提交，无需确认对话框，归档集合回声落地后该行从所有分组视图中消失；归档当前会话会用一个新的独立会话替代它，而后续导航可以取代这次替代。Fork 在源会话最后一个已完成轮次处 fork，在客户端递增继承的持久化标题后再打开子会话。Workspace 行内的 Delete 操作会打开确认框，说明保留边界；成功后该分组被移除，其 Session 则留在 Ungrouped 下。
...
`ctx.uiWorkspace.openSession(id)` 会选中会话，并让主区域返回会话界面；这两项构成一次 UI 导航操作，即使目标会话已经是当前会话也同样执行。`openWorkspace(id, beforeOpen?)` 和 `forkSession(id)` 仅在请求未被后续导航替代时打开结果；全局新会话创建独立会话。可选的同步准备回调仅对仍有效的工作区请求执行，因此过期请求不会搬移 composer 草稿。后续导航或所有者释放会阻止晚到的 UI 提交，但不取消底层会话创建。选中失败时保留当前全局面板。会话行读取 `usePanelInfo`，在全局面板活跃时不显示会话选中样式；仅把焦点移到搜索框或目录选择器不会离开该面板。

有两种非用户手势的落地：启动后的首次选中，以及归档当前会话后欠下的替代会话。两者都会选中与 `openChat` 相同的独立会话，但不新开导航世代、也不激活会话界面，因此用户已有的导航或全局面板保持原样。只要用户仍拥有某次导航在进行中，落地就会被推迟——该导航拥有结果，其失败会重新欠下这次落地；被取代的导航在取消时立即释放这一主张，即使它自己的创建尚未结束。
