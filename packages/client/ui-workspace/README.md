...
The sidebar separates **Sessions in projects** from **Sessions outside projects** with one heading per section: project rows and their Sessions fill the first, and Sessions outside every project tile directly under the second with no folder row of their own. **New session** creates a normal Agent session with its own working directory and no project selection; a project row’s **+** creates or reuses a blank session in that project directory. Right-clicking a Session row, or its trailing ···, opens Rename, **Move to project** (every registered project, outside projects, or a new project through the directory picker), Fork, Pin, and Archive; hovering the row exposes Pin and Archive as quick actions. Moving changes project membership only: files, working directory, history, and Agent composition remain unchanged.
...
The Session row's Rename action opens a dialog prefilled with the row's display title; confirming an unchanged title is deliberately allowed — it pins the current automatic title against regeneration. **Move to project** changes membership directly for a registered project or the outside bucket, and opens the directory picker for a project that must be registered first; that picker's dialog returns only when the picked directory could not be adopted, where it reports the failure and still offers every target. Archive commits from the row's quick action or menu without a confirmation dialog and the row disappears from every grouping surface when the archive-set echo lands; archiving the current Session replaces it with a fresh independent Session, which a later navigation supersedes. Fork forks at the source's last completed turn, increments the inherited persisted title on the client, and then opens the child. Workspace Delete opens a confirmation that states the retention boundary; success removes the group while its Sessions remain under Ungrouped.
...
`ctx.uiWorkspace.openSession(id)` selects the Session and returns the main area to the Conversation as one UI navigation action, including when that Session was already current. `openWorkspace(id, beforeOpen?)` and `forkSession(id)` open their result only if no later navigation has superseded the request; global New Session creates an independent session. The optional synchronous preparation callback runs only for a current Workspace request, so superseded requests do not move composer drafts. Navigation or owner disposal suppresses the late UI commit, not the underlying Session creation. Selection failure leaves a global panel visible. Session rows read `usePanelInfo` to suppress their selected appearance while a global panel is active; search and directory-picker focus alone do not leave that panel.

Two landings happen without a user gesture: the first selection after startup, and the replacement an archived current Session owes. Both select the independent Session `openChat` would create without starting a navigation generation and without activating the Conversation surface, so a navigation or global panel the user already has is left exactly as it is. A landing also defers while a navigation the user still owns is in flight — that navigation owns the outcome, and a failure re-owes the landing. A superseded navigation releases that claim as soon as it is cancelled, even while its own creation is still unresolved.
