...
import type { RowDragProps, SessionMoveTargets } from '../src/client/rows/Rows.tsx'
...
/** Row-level project targets and quick actions every fixture needs. */
const moveStub: SessionMoveTargets = { projects: [], moveTo: vi.fn(), moveToNewProject: vi.fn() }
...
describe('workspace browser rows', () => {
  it('omits only an empty leading status slot in the hierarchy-free flat list', () => {
    const idle: SessionNode = {
      ...
      runningSubagentCount: 0, completed: false, hasActiveSchedule: false, pinned: false, updatedAt: 0,
    }
    const view = render(<SessionNodeItem node={idle} currentId={undefined} now={0} onOpen={vi.fn()}
      onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onTogglePin={vi.fn()} move={moveStub} flat t={t} />)
    ...
    view.rerender(<SessionNodeItem node={{ ...idle, running: true }} currentId={undefined} now={0}
      onOpen={vi.fn()} onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onTogglePin={vi.fn()} move={moveStub} flat t={t} />)
  ...
  it('renders and opens a selected running Session row', () => {
    ...
    render(
      <SessionNodeItem node={node} currentId={node.id} now={0} onOpen={onOpen}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onTogglePin={vi.fn()} move={moveStub} t={t} />,
    )
  ...
  it('keeps the active-Schedule marker between the title and time in grouped and flat rows', () => {
    ...
    const node: SessionNode = {
      ...
      runningSubagentCount: 0, completed: false, hasActiveSchedule: true, pinned: false, updatedAt: 0,
    }
    const view = render(
    ...
    const renderRow = (over: Partial<SessionNode>) => render(
      <SessionNodeItem
        node={{
          ...
          runningSubagentCount: 0, completed: false, hasActiveSchedule: false, pinned: false, updatedAt: 0, ...over,
        }}
  ...
  it('shows descendant activity without describing an idle parent as running', () => {
    ...
    try {
      const node: SessionNode = {
        ...
        runningSubagentCount: 2, completed: false, hasActiveSchedule: false, pinned: false, updatedAt: 0,
      }
  ...
  it('keeps descendant activity secondary while the parent is running', () => {
    ...
    try {
      const node: SessionNode = {
        ...
        runningSubagentCount: 1, completed: false, hasActiveSchedule: false, pinned: false, updatedAt: 0,
      }
      render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onTogglePin={vi.fn()} move={moveStub} t={t} />)
      const row = screen.getByRole('treeitem')
  ...
  it('keeps child activity as a secondary status while user attention is primary', () => {
    const node: SessionNode = {
      ...
      running: false, runningSubagentCount: 1, completed: false, hasActiveSchedule: false, pinned: false, updatedAt: 0,
    }
    render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
      onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onTogglePin={vi.fn()} move={moveStub} t={t} />)
    const row = screen.getByRole('treeitem')
  ...
  it('session row menu opens without opening the session and dispatches rename, fork, pin, move, and archive', () => {
    const onOpen = vi.fn()
    const onRename = vi.fn()
    const onFork = vi.fn()
    ...
    const moveTo = vi.fn()
    const moveToNewProject = vi.fn()
    const node: SessionNode = {
      id: sid('s1'), title: 'One', blank: false, running: false,
      runningSubagentCount: 0, completed: false, hasActiveSchedule: false, pinned: false, updatedAt: 0,
    }
    render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={onOpen}
      onRename={onRename} onFork={onFork} onArchive={onArchive} onTogglePin={onTogglePin}
      move={{ projects: [{ workspaceId: wid('alpha'), title: 'Alpha' }], moveTo, moveToNewProject }} t={t} />)
    ...
    // Pin toggles browser-local viewing state; the row never opens.
    fireEvent.click(screen.getByRole('button', { name: '会话“One”的操作' }))
    fireEvent.click(screen.getByRole('menuitem', { name: '置顶会话' }))
    expect(onTogglePin).toHaveBeenCalledWith(node.id)
    // Move submenu: one registered project, the outside bucket, and the new-project flow.
    fireEvent.click(screen.getByRole('button', { name: '会话“One”的操作' }))
    fireEvent.click(screen.getByRole('menuitem', { name: '移动到项目…' }))
    fireEvent.click(screen.getByRole('menuitem', { name: 'Alpha' }))
    expect(moveTo).toHaveBeenCalledWith(node.id, wid('alpha'))
    fireEvent.click(screen.getByRole('button', { name: '会话“One”的操作' }))
    fireEvent.click(screen.getByRole('menuitem', { name: '移动到项目…' }))
    fireEvent.click(screen.getByRole('menuitem', { name: '移到项目外' }))
    expect(moveTo).toHaveBeenLastCalledWith(node.id, undefined)
    fireEvent.click(screen.getByRole('button', { name: '会话“One”的操作' }))
    fireEvent.click(screen.getByRole('menuitem', { name: '移动到项目…' }))
    fireEvent.click(screen.getByRole('menuitem', { name: '新建项目…' }))
    expect(moveToNewProject).toHaveBeenCalledWith(node.id)
    expect(onRename).toHaveBeenCalledOnce()
    expect(onOpen).not.toHaveBeenCalled()
    // Escape closes without selecting (Menu onClose path).
    fireEvent.click(screen.getByRole('button', { name: '会话“One”的操作' }))
  ...
  it('right-click opens the same session menu at the pointer, and the hover quick actions stay direct', () => {
    const onOpen = vi.fn()
    const onRename = vi.fn()
    const onArchive = vi.fn()
    const onTogglePin = vi.fn()
    ...
    render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={onOpen}
      onRename={onRename} onFork={vi.fn()} onArchive={onArchive} onTogglePin={onTogglePin}
      move={moveStub} t={t} />)

    fireEvent.click(screen.getByRole('button', { name: '置顶会话“One”' }))
    expect(onTogglePin).toHaveBeenCalledWith(node.id)
    fireEvent.click(screen.getByRole('button', { name: '归档会话“One”' }))
    expect(onArchive).toHaveBeenCalledWith(node.id)
    expect(onOpen).not.toHaveBeenCalled()

    fireEvent.contextMenu(screen.getByRole('treeitem'), { clientX: 120, clientY: 40 })
    expect(onOpen).not.toHaveBeenCalled()
    const menu = screen.getByRole('menu')
    // Portal placement measures the pointer rect instead of the ellipsis
    // trigger; the list then opens 4px below that rect (Menu's bottom side).
  ...
  it('a pinned row keeps its pin toggle on screen and offers Unpin in the menu', () => {
    const onTogglePin = vi.fn()
    const pinnedNode: SessionNode = {
      id: sid('s1'), title: 'Pinned', blank: false, running: false,
      runningSubagentCount: 0, completed: false, hasActiveSchedule: false, pinned: true, updatedAt: 0,
    }
    render(<SessionNodeItem node={pinnedNode} currentId={undefined} now={0} onOpen={vi.fn()}
      onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onTogglePin={onTogglePin}
      move={moveStub} t={t} />)
    const actions = screen.getByRole('button', { name: '取消置顶会话“Pinned”' }).parentElement as HTMLElement
    expect(actions.className).toMatch(/rowActionsPinned/)
    expect(screen.getByRole('treeitem').className).toMatch(/pinnedRow/)
    fireEvent.click(screen.getByRole('button', { name: '会话“Pinned”的操作' }))
    fireEvent.click(screen.getByRole('menuitem', { name: '取消置顶' }))
    expect(onTogglePin).toHaveBeenCalledWith(pinnedNode.id)
  })
  ...
  it.each([
    ...
    try {
      const node: SessionNode = {
        ...
        pendingInteraction, running: true, runningSubagentCount: 0, completed: false,
        hasActiveSchedule: false, pinned: false, updatedAt: 0,
      }
      ...
      view.rerender(<SessionNodeItem node={{ ...node, running: false }} currentId={undefined} now={0}
        onOpen={vi.fn()} onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onTogglePin={vi.fn()} move={moveStub} t={t} />)
  ...
  it('completed hover card shows the Completed status line', () => {
    ...
    try {
      const node: SessionNode = {
        ...
        runningSubagentCount: 0, completed: true, hasActiveSchedule: false, pinned: false, updatedAt: 0,
      }
      render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onTogglePin={vi.fn()} move={moveStub} t={t} />)
      fireEvent.pointerEnter(screen.getByRole('treeitem').parentElement as HTMLElement)
  ...
  it('draggable row wires start/end and gates hover/drop on an active same-group drag', () => {
    ...
    const { rerender } = render(
      <SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onTogglePin={vi.fn()} move={moveStub} drag={inactive} t={t} />,
    )
    const row = screen.getByRole('treeitem')
    stubRect(row)
    ...
    rerender(
      <SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onTogglePin={vi.fn()} move={moveStub} drag={active} t={t} />,
    )
    ...
    rerender(
      <SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onTogglePin={vi.fn()} move={moveStub} drag={after} t={t} />,
    )
