...
The composer accepts independent sessions without Workspace membership. Its working-directory button shows the actual execution directory and opens that folder through the Host. Project membership changes do not retarget this directory. Normal Agent presets remain available for new independent sessions.
...
The main Conversation follows the available layout width without draggable transcript handles or a persisted width override. The `widthControls` Factory position remains available to alternate compositions.
