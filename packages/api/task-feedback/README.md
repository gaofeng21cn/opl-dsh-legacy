...
Tracks dispatched DSH tasks and delivers bounded outcome notifications to their calling sessions. Durable task and receipt records prevent duplicate review claims and recover interrupted receivers. Approval and question notifications retain the user decision; bounded recovery applies only to the documented reasoning replay failure.
...
### Task outcome notifications

#### What the model sees

The service observes `turn/end` and related Session state. It does not add prompt input. Configured wake transports can deliver bounded, untrusted task summaries to the caller.

#### Token effect

The service makes no model call. Only a receiver that chooses to process a delivered notification spends a model turn; progress-only transitions do not notify by default.

#### KV Cache effect

No direct cache changes. A receiving Session appends any accepted notification through its own normal message path.
