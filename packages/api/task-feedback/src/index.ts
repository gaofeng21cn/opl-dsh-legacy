...
export default class TaskFeedbackService extends TypertRemoteService {
  ...
  private recordedResumeInstruction(session: Session, requestId: string): { seq: number; turn: number | null } | undefined {
    const events = this.sessionEvents(session)
    let instructionSeq: number | null = null
    let claiming: number | null = null
    let open: number | null = null
    for (const event of events) {
      if (event.type === 'turn/start') { open = event.data.turn; continue }
      if (event.type === 'turn/end') { open = null; continue }
      if (instructionSeq !== null || !isResumeInstruction(event, requestId)) continue
      instructionSeq = Number(event.seq)
      claiming = open
    }
    if (instructionSeq === null) return undefined
    if (claiming === null) {
      for (const event of events) {
        if (event.type === 'turn/start' && Number(event.seq) > instructionSeq) {
          claiming = event.data.turn
          break
        }
      }
    }
    return { seq: instructionSeq, turn: claiming }
  }

  /** The Session's recorded events, as the resume-binding read consumes them. */
  private sessionEvents(session: Session): readonly SessionEvent[] {
  ...
  private decideAttemptCatchUp(record: TaskRecordState, session: Session): TaskRecordState | undefined {
    ...
    const wait: SessionWaitState | undefined = this.ctx.get('sessionProjections')?.stateOf(session, 'sessionWait')
  ...
  private completedTurnLeakage(session: Session, turn: number): string[] {
    ...
    for (let index = events.length - 1; index >= 0; index -= 1) {
      ...
      return leakedToolSyntaxFamilies(event.data.message.content)
    }
    return []
  }
