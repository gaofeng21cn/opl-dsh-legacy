...
import {
  ReasoningEffortId, assistantStreamChunks, createSystemMessage, createUserMessage, freezeMessage,
...
export class SessionCommandController {
  ...
  private async rewindOnce(request: SessionRewindRequest): Promise<SessionRewindValue> {
    ...
    const replacement = agent.session.append('system/message', {
      turn: coordinates.turn,
      step: coordinates.step,
      message: createSystemMessage('', REWIND_SURFACE_PLUGIN),
    }, {
   ...
   */
  private requireServedRoute(agent: Agent): void {
    const selection = this.agents.selectionFor(agent).current
    if (routeServed(this.ctx, selection.provider)) return
    throw new RemoteError(
      'session/model-unavailable',
